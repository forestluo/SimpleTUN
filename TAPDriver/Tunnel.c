...
_BOOL NetworkL2TPOut(NetworkTunnel* pTunnel,IPPacket* lpPacket)
{
	...
	//Add read size.
	pTunnel->nOutputSize += lpPacket->udpPacket.buffer.nCount;
	//Return true.
	return _TRUE;
}
...
_BOOL NetworkWriteProcess(_OBJECT lpObject)
{
	...
	InitializeIPPacket(&packet.ipPacket);
	//Set IP packet.
	packet.ipPacket.nID = rand() & 0x7FFFFFFF;
	//Set protocol.
	...
	//Add input size.
	pTunnel->nInputSize += buffer.nCount;
	//Return true.
	return _TRUE;
}
...
////////////////////////////////////////////////////////////////////////////////
//
// Open socket of TAP network tunnel.
//
////////////////////////////////////////////////////////////////////////////////

_BOOL OpenNetworkTunnelSocket(NetworkTunnel* pTunnel)
{
#ifdef _DEBUG
	assert(pTunnel != NULL);
#endif
	...
	{
#ifdef _DEBUG
		LogRequestedEvent("Tunnel::OpenNetworkTunnelSocket : fail to connect server !",
			LOG_TUNNEL | LOG_EXCEPTION);
#endif
		return _FALSE;
	}
#ifdef _DEBUG
	LogRequestedEvent("Tunnel::OpenNetworkTunnelSocket : server was connected !",
		LOG_TUNNEL | LOG_INFORMATION);
#endif
	...
	{
#ifdef _DEBUG
		LogRequestedEvent("Tunnel::OpenNetworkTunnelSocket : fail to startup write thread !",
			LOG_TUNNEL | LOG_EXCEPTION);
#endif
		return _FALSE;
	}
#ifdef _DEBUG
	LogRequestedEvent("Tunnel::OpenNetworkTunnelSocket : read thread was startup !",
		LOG_TUNNEL | LOG_INFORMATION);
#endif
	...
	{
#ifdef _DEBUG
		LogRequestedEvent("Tunnel::OpenNetworkTunnelSocket : fail to startup read thread !",
			LOG_TUNNEL | LOG_EXCEPTION);
#endif
		return _FALSE;
	}
...
_BOOL OpenTAPNetworkTunnel(NetworkTunnel* pTunnel)
{
	...
	nLength = 0;
	//Set config.
    if(!DeviceIoControl(pTunnel->lpHandle,TAP_IOCTL_CONFIG_POINT_TO_POINT,
			    pTunnel->ipLocal,2 * sizeof(_IPV4),
			    pTunnel->ipLocal,2 * sizeof(_IPV4),&nLength,NULL))
	{
#ifdef _DEBUG
		LogRequestedEvent("Tunnel::OpenTAPNetworkTunnel : fail to set point to point !",
			LOG_TUNNEL | LOG_EXCEPTION);
#endif
		return _FALSE;
	}
#ifdef _DEBUG
	LogRequestedEvent("Tunnel::OpenTAPNetworkTunnel : point to point was enabled !",
		LOG_TUNNEL | LOG_INFORMATION);
#endif
	*/

	//Copy values.
	memcpy(values,pTunnel->ipLocal,4 * sizeof(_UINT32));
	//Set remote mask.
	memcpy(&values[3],pTunnel->ipNetmask,sizeof(_UINT32));

	//Set length.
	nLength = 0;
	//Set config.
    if(!DeviceIoControl(pTunnel->lpHandle,TAP_IOCTL_CONFIG_DHCP_MASQ,
			    values,4 * sizeof(_IPV4),
			    values,4 * sizeof(_IPV4),&nLength,NULL))
	{
#ifdef _DEBUG
		LogRequestedEvent("Tunnel::OpenTAPNetworkTunnel : fail to config DHCP !",
			LOG_TUNNEL | LOG_EXCEPTION);
#endif
		return _FALSE;
	}
#ifdef _DEBUG
	LogRequestedEvent("Tunnel::OpenTAPNetworkTunnel : DHCP was configurated !",
		LOG_TUNNEL | LOG_INFORMATION);
#endif
	...
	//Return true.
	return OpenNetworkTunnelSocket(pTunnel);
}
...
void CloseNetworkTunnelSocket(NetworkTunnel* pTunnel)
{
#ifdef _DEBUG
	assert(pTunnel != NULL);
#endif
...
#ifdef _DEBUG
	LogRequestedEvent("Tunnel::CloseNetworkTunnelSocket : shutdown read thread !",
		LOG_TUNNEL | LOG_INFORMATION);
#endif
...
#ifdef _DEBUG
	LogRequestedEvent("Tunnel::CloseNetworkTunnelSocket : shutdown write thread !",
		LOG_TUNNEL | LOG_INFORMATION);
#endif
	...
	{
#ifdef _DEBUG
		LogRequestedEvent("Tunnel::CloseNetworkTunnelSocket : socket was closed !",
			LOG_TUNNEL | LOG_INFORMATION);
#endif
		//Close socket.
...
void CloseNetworkTunnel(NetworkTunnel* pTunnel)
{
	...
	//Close network tunnel socket.
	CloseNetworkTunnelSocket(pTunnel);
