...
_UINT32 GetAdapterIndexByGuid(_STRING lpstrGuid)
{
	...
	//Format.
	sprintf((_STRING)strName,"\\DEVICE\\TCPIP_%s",lpstrGuid);
	//Check index.
	while(nIndex < theInterfaceInformationList->NumAdapters)
	{
		//Set flag.
		nFlag = 0;
		//Check name.
		for(i = 0;i < wcslen(theInterfaceInformationList->Adapter[nIndex].Name);i ++)
		{
			//Check name.
			if(theInterfaceInformationList->Adapter[nIndex].Name[i] != strName[i])
			{
				//Set nFlag.
				nFlag = 1;break;
			}
